...
import 'package:vizitland_partner/models/appointmentHistoryModel.dart';
import 'package:vizitland_partner/models/bookingConfirmModel.dart';
import 'package:vizitland_partner/models/bookingDetailModel.dart';
import 'package:vizitland_partner/models/businessLayer/apiResult.dart';
import 'package:vizitland_partner/models/businessLayer/global.dart' as global;
import 'package:vizitland_partner/models/couponModel.dart';
import 'package:vizitland_partner/models/currencyModel.dart';
import 'package:vizitland_partner/models/expertModel.dart';
import 'package:vizitland_partner/models/faqModel.dart';
import 'package:vizitland_partner/models/galleryModel.dart';
import 'package:vizitland_partner/models/homePageModel.dart';
import 'package:vizitland_partner/models/myWalletModel.dart';
import 'package:vizitland_partner/models/notificationModel.dart';
...
import 'package:vizitland_partner/models/privacyAndPolicyModel.dart';
import 'package:vizitland_partner/models/productModel.dart';
import 'package:vizitland_partner/models/productOrderModel.dart';
import 'package:vizitland_partner/models/reviewModel.dart';
import 'package:vizitland_partner/models/serviceListModel.dart';
...
import 'package:vizitland_partner/models/userRequestModel.dart';
import 'package:vizitland_partner/models/vendorServiceModel.dart';
...
class APIHelper {
  ...
  Future<dynamic> addExpert(
      ...
      int staffVarientEditable,
      int staffServiceFor,
      String staffVarient,
      String staffDescription,
      File _image,
      File _certificate) async {
    try {
      Response response;
      var dio = Dio();
      var formData = FormData.fromMap({
        ...
        'staff_mobile': staffPhone,
        'password': staffPassword,
        'varient_editable': staffVarientEditable,
        'service_for': staffServiceFor,
        'varient': staffVarient,
        ...
        'staff_image': _image != null
            ...
            : null,
        'staff_certificate': _certificate != null
            ? await MultipartFile.fromFile(
                _certificate.path.toString(),
                //contentType: MediaType("image", "jpeg"),
              )
            : null,
      });
  ...
  Future<dynamic> addService(int id, String serviceName, String _varient,
      int accessVarientId, String serviceImage) async {
    try {
      Response response;
      var dio = Dio();
      var formData = FormData.fromMap({
        'vendor_id': id,
        'product_name': "${serviceName},${accessVarientId}",
        'varient': _varient,
        'service_image': serviceImage != null ? serviceImage : null,
  ...
  Future<dynamic> addServiceVariant(ServiceVariant serviceVariant, int access_varient_id) async {
    try {
      final response = await http.post(
        Uri.parse("${global.baseUrl}add_servicevariant"),
        headers: await global.getApiHeaders(false),
        body: json.encode({
          'vendor_id': serviceVariant.vendor_id,
          'service_id': serviceVariant.service_id,
          'varient_id': serviceVariant.varient_id,
          'varient': serviceVariant.varient,
          'price': serviceVariant.price != null ? serviceVariant.price : null,
          'time': serviceVariant.time != null ? serviceVariant.time : null,
          'access_service_id': access_varient_id != null ? access_varient_id : null,
        }),
      );
  ...
  Future<dynamic> editService(int vendorId, String serviceName, String _varient,
      int accessVarientId, String serviceImage, int serviceId) async {
    try {
      Response response;
      var dio = Dio();
      var formData = FormData.fromMap({
        'vendor_id': vendorId,
        'service_name': "${serviceName},${accessVarientId}",
  ...
  Future<dynamic> getVarients(int id) async {
    try {
      final response = await http.post(
        Uri.parse("${global.baseUrl}get_partner_varients"),
        headers: await global.getApiHeaders(false),
        body: json.encode({"vendor_id": id}),
      );
      ...
      if (response.statusCode == 200) {
        if (json.decode(response.body)["data"] != null) {
          recordList = List<PartnerVarientModel>.from(json
              .decode(response.body)["data"]
              .map((x) => PartnerVarientModel.fromJson(x)));
          // final data = json.decode(response.body)["data"];
          // print(data.map((x) => PartnerVarientModel.fromJson(x).toJson()));
  ...
  Future<dynamic> getVendorService(int id) async {
    try {
      final response = await http.post(
        Uri.parse("${global.baseUrl}get_vendor_service"),
        headers: await global.getApiHeaders(false),
        body: json.encode({"vendor_id": id}),
      );
